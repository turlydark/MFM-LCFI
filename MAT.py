...
from frequency_model import FreProcessing
...
class ThreeShallowLayerMAT(nn.Module):
    def __init__(self, net='xception',feature_layer1='b1',feature_layer2='b2',feature_layer3='b3',attention_layer='final',num_classes=2, M=8,mid_dims=256,\
    dropout_rate=0.5,drop_final_rate=0.5, pretrained=False,alpha=0.05,size=(380,380),margin=1,inner_margin=[0.01,0.02]):
        ...
        self.projection_local=nn.Sequential(nn.Linear(M*(self.num_features1+self.num_features2+self.num_features3),mid_dims),nn.Hardswish(),nn.Linear(mid_dims,mid_dims))
        ...
        self.project_final=nn.Linear(layers['final'].shape[1],mid_dims)
        self.ensemble_classifier_fc=nn.Sequential(nn.Linear(mid_dims*2,mid_dims),nn.Hardswish(),nn.Linear(mid_dims,num_classes))
        # print(self.ensemble_classifier_fc)
        # exit(0)
        # Sequential(
    ...
    def train_batch(self,x,y,jump_aux=False,drop_final=False):
        ...
        ensemble_logit=self.ensemble_classifier_fc(feature_matrix)
        ensemble_loss=F.cross_entropy(ensemble_logit,y)
        return dict(ensemble_loss=ensemble_loss,aux_loss=aux_loss,attention_maps=attention_maps_,ensemble_logit=ensemble_logit,feature_matrix=feature_matrix_1,feature_matrix_d=feature_matrix_d)
