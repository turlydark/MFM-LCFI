...
class ThreeShallowLayerMATwithLAD(nn.Module):
    def __init__(self, model_name='efficientnet-b4'):
        super(ThreeShallowLayerMATwithLAD, self).__init__()
        self.threeMat = ThreeShallowLayerMAT(model_name, size=(299, 299))
        self.freModel = FreProcessing(device="cuda")
...
if __name__ == "__main__":
    model = ThreeShallowLayerMATwithLAD()
    dummy = torch.rand((1, 3, 299, 299))
    out  = model(dummy)
    print(out)

    # model = FreProcessing(device="cuda")
    # # print(model)
    # dummy = torch.rand((1, 3, 299, 299))
    # out  = model(dummy)
    # print(out)
